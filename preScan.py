...
import string
import SbScan as sb
from SbScan import wj, p, B, chk, authorize, to_spisok, set_filter
...
try:
    ...
    while g < 1000:
        ...
        read_cursor.execute('SELECT data_id, inn, kpp FROM debug WHERE data_id >-1;')
    ...
    #            xc_dataid(driver,'data_id',str(firm.get_attribute('data-id')))
                ...
                sql = 'INSERT INTO debug(data_id, inn, kpp, firm_full_name, gen_info, act_num, act_list, ch_title, ' \
                ...
                dbconn.commit()
                read_cursor.execute('SELECT count(*) FROM debug WHERE data_id >-1;')
                rows = read_cursor.fetchall()
